...
//	TFile *file = new TFile("RUN511.ch.root","read");
	TFile *file1 = new TFile("RUN515.ch.root","read");
	TTree *tree = (TTree *)file1 -> Get("channels");
	TFile *write = new TFile("constant_of_det1.root","RECREATE");	
	Double_t a;
	Short_t detector,channel;
	TTree  *t1 = new TTree("tree","constants");		
	t1 -> Branch("a",&a,"a/D");
	...
	for(auto det :{1}){
			...
			for(auto dch = 1; dch <=11; dch++){
				

				if (dch == 6|| dch == 7 || dch == 8){
					x1 = 1600;
					x2 = 2300;
				}
				
				
				if (dch == 1 || dch == 2 || dch == 9 || dch == 11){
					x1 = 1800;
					x2 = 2300;
				}

				if (dch == 2 || dch == 9 || dch == 11){
					x1 = 1800;
					x2 = 2300;
				}
				...
				TString namehist = Form("hist_d%d_%d", det, dch);
